...
class OriginPowerAILargeLanguageModel(OAICompatLargeLanguageModel):
    ...
    def _needs_max_completion_tokens(m: str) -> bool:
        return bool(OriginPowerAILargeLanguageModel._NEEDS_MAX_COMPLETION_TOKENS_PATTERN.match(m))

    def _merge_credentials(self, model: str, credentials: dict) -> dict:
        """Fill credential defaults for predefined (provider-level) models.

        The OAICompat base class reads mode / function-calling / vision settings
        from credentials, which are only present for customizable-model
        credentials. For predefined models Dify passes the provider credentials
        instead, so derive the missing keys from the bundled model YAML.
        """
        ...
        if schema:
            properties = schema.model_properties or {}
            if properties.get(ModelPropertyKey.MODE):
                merged["mode"] = properties[ModelPropertyKey.MODE]
            if properties.get(ModelPropertyKey.CONTEXT_SIZE):
                merged["context_size"] = properties[ModelPropertyKey.CONTEXT_SIZE]
            features = set(schema.features or [])
            if ModelFeature.TOOL_CALL in features or ModelFeature.MULTI_TOOL_CALL in features:
                merged.setdefault("function_calling_type", "tool_call")
            if ModelFeature.STREAM_TOOL_CALL in features:
                merged.setdefault("stream_function_calling", "supported")
            if ModelFeature.VISION in features:
                merged.setdefault("vision_support", "support")
        merged.setdefault("mode", "chat")
        return merged
    ...
    def validate_credentials(self, model: str, credentials: dict) -> None:
        ...
        """
        credentials = self._merge_credentials(model, credentials)
